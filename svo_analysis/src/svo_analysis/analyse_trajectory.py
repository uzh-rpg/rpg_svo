...
def analyse_trajectory(results_dir, n_align_frames = 20, use_hand_eye_calib = True):
    ...
    if not os.path.isfile(os.path.join(results_dir, 'translation_error.txt')):
        ...
        T_cm, T_mc = load_hand_eye_calib(results_dir)
        print('loading datasets.')        
        t_es, p_es, q_es, t_gt, p_gt, q_gt = load_dataset(results_dir)
...
def load_hand_eye_calib(results_dir):
    P = yaml.load(open(os.path.join(results_dir, 'dataset_params.yaml'),'r'))['hand_eye_calib']
    
    T_cm_quat = np.array([P['Tcm_qx'], P['Tcm_qy'], P['Tcm_qz'], P['Tcm_qw']])
    T_cm_tran = np.array([P['Tcm_tx'], P['Tcm_ty'], P['Tcm_tz']])
...
def load_dataset(results_dir):
    """
    Assumes that groundtruth measurements are already matched with the
    images. 
    """

    data_gt = open(os.path.join(results_dir, 'groundtruth_matched.txt')).read()
    ...
    matches = associate.associate(data_gt, data_es, 0.0, 0.0001)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_align_frames', help='', default=20)
